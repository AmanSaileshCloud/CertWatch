...
def test_patch_updates_config(client):
    c, store = client
    r = c.patch("/domains/ex.com:443", json={"alerts_enabled": False})
    assert r.status_code == 200
    body = r.json()
    assert body["alerts_enabled"] is False
    assert "notify_emails" not in body
